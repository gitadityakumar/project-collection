...
const Header = () => {
  ...
  const NavLink = ({title})=> (
    <LinkScroll to={title}offset={-100} spy smooth onClick={()=>setIsOpen(false)} activeClass='nav-active' className='base-bold text-p4 uppercase transition-colors duration-500 cursor-pointer hover:text-p1 max-lg:my-4 max-lg:h5'>
    {title}
    </LinkScroll>
  );
